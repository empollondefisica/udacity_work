...
def cellular_automaton(start, pattern, generations):
    values = get_pattern(pattern)    
    ...
    while count < generations:
        ...
        for index in range(0, len(start)):
            parent = curr_gen[index - 1] + curr_gen[index] + curr_gen[(index + 1) % len(curr_gen)]
            next_gen = next_gen + values[get_number(parent)]
        curr_gen = next_gen
        count = count + 1
    return curr_gen
